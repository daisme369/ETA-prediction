...
MODEL_NAME = "hour_bin_xgb_residual_eta"
...
def main() -> None:
    ...
    configure_mlflow(config, "eta_fixed_trip_hour_bin_xgboost_residual")
    ...
    with mlflow.start_run(run_name=MODEL_NAME):
        log_run_context(
            config,
            prepared,
            model_type="hour_bin_xgboost",
            target_type="residual_secs",
            extra_params={"xgboost": config.get("xgboost", {}), "hour_binning": prepared["hour_binning"]},
        )
        ...
        save_joblib_model(paths, f"{MODEL_NAME}.joblib", {"preprocessor": preprocessor, "model": model, "features": features})

        fi = _feature_importance(preprocessor, model)
        fi_path = paths["metrics_dir"] / f"{MODEL_NAME}_feature_importance.csv"
        fi.to_csv(fi_path, index=False)
        mlflow.log_artifact(str(fi_path))

        residual_path = paths["plots_dir"] / f"{MODEL_NAME}_target_vs_prediction.csv"
